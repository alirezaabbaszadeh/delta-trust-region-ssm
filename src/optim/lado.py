...
def compute_lr_delta(lr_base: float, seq_len: int, l_ref: int, alpha: float) -> float:
    ...
    if scale > 1.0:
        scale = 1.0
    return float(lr_base) * float(scale)
...
def build_two_group_adamw(
    ...
    *,
    ...
    if not hasattr(model, "delta_parameters"):
        raise ValueError("Model must define delta_parameters() to use two-group optimizer.")
    ...
    optimizer = torch.optim.AdamW(
        [
            {"params": other_params, "lr": float(lr_other), "weight_decay": float(weight_decay)},
            {"params": delta_params, "lr": float(lr_delta), "weight_decay": float(delta_weight_decay)},
        ]
    )
    return optimizer, 1
...
def clip_delta_grads(delta_params: list[torch.nn.Parameter], max_norm: float) -> float:
    return float(torch.nn.utils.clip_grad_norm_(delta_params, max_norm=float(max_norm)))
